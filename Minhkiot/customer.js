...
let currentCategory = '';
let menuData = [];
let selectedTable = ''; // <-- THÊM BIẾN NÀY
...
function updateLanguage() {
    const translations = {
        vi: {
            ...
            close: "Đóng",
            takeawayBtn: "Mang về (Takeaway)" // Thêm bản dịch cho nút takeaway
        },
        en: {
            ...
            close: "Close",
            takeawayBtn: "Takeaway" // Thêm bản dịch cho nút takeaway
        },
        fi: {
            ...
            close: "Sulje",
            takeawayBtn: "Nouto" // Thêm bản dịch cho nút takeaway
        }
    };

    const currentTranslations = translations[currentLanguage];
    
    // Update các element với kiểm tra null
    ...
    // Update table selection modal title
    ...
    // ===== CẬP NHẬT TEXT CHO NÚT TAKEAWAY =====
    // Update takeaway button text
...
// Được gọi khi bấm nút "Đặt Món"
function startCheckoutProcess() {
    if (cart.length === 0) {
        showNotification(getEmptyCartMessage(), 'warning');
        return;
    }
    // Cập nhật tiêu đề modal chọn bàn theo ngôn ngữ hiện tại
    const tableSelectionTitleElement = document.querySelector('#tableSelectionModal .modal-header h2');
    if (tableSelectionTitleElement) {
        tableSelectionTitleElement.textContent = getTableSelectionTitle();
    }
    ...
    document.getElementById('tableSelectionModal').classList.add('show');
}
...
// Được gọi khi bấm vào một ô tròn (bàn)
function selectTable(tableId) {
    selectedTable = tableId; // Lưu lại bàn đã chọn
    console.log("Đã chọn:", selectedTable);
    
    // Ẩn modal chọn bàn
    closeTableSelection();
    
    // Mở modal xem lại đơn hàng (bước tiếp theo như cũ)
    showOrderReview();
}

// ===== THÊM HÀM MỚI NÀY VÀO =====
// Được gọi khi bấm vào nút "Mang về"
function selectTakeaway() {
    selectedTable = 'Takeaway'; // Lưu lại lựa chọn
    console.log("Đã chọn:", selectedTable);
    
    // Ẩn modal chọn bàn/takeaway
    closeTableSelection();
...
async function confirmOrder() {
    ...
    const orderData = {
        ...
        // SỬA DÒNG NÀY
        tableNumber: selectedTable, // Thay vì "Bàn 1"
...
document.addEventListener('click', function(e) {
    ...
    // THÊM ĐÓNG MODAL CHỌN BÀN KHI CLICK NGOÀI
    if (e.target === document.getElementById('tableSelectionModal')) {
        closeTableSelection();
    }
});
